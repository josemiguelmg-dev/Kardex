...
export default function Configuracion({ permisoNotificaciones, setPermisoNotificaciones, user, doctor }) {
  ...
  const registrarHuellaGlobal = async () => {
    try {
      ...
      await navigator.credentials.create({
        publicKey: {
          challenge, 
          rp: { name: "Kardex", id: window.location.hostname },
      ...
      setTieneHuella(true); 
      setMensaje({ tipo: 'exito', texto: '¡Huella dactilar activada correctamente!' });
    } catch (error) { 
      setMensaje({ tipo: 'error', texto: 'El registro biométrico fue cancelado o no es compatible.' }); 
    } finally { 
  ...
  const solicitarNotificacionesManual = () => { 
    if ("Notification" in window) {
      Notification.requestPermission().then(perm => {
        ...
        if (perm === 'granted') {
          setMensaje({ tipo: 'exito', texto: '¡Alertas activadas exitosamente!' });
        } else {
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-in relative pb-10">
      ...
      {mensaje.texto && (
        <div className={`mb-5 px-4 py-3 rounded-xl border transition-all ${mensaje.tipo === 'error' ? 'bg-red-900/40 border-red-500/50 text-red-200' : 'bg-cyan-900/40 border-cyan-500/50 text-cyan-200'}`}>
      ...
      <div className="bg-[#141824] p-5 rounded-2xl border border-white/10 shadow-md flex flex-col sm:flex-row items-center sm:justify-between gap-4 mb-5">
        <div className="flex items-center gap-4 w-full">
          ...
          <div className="text-left">
            ...
            <p className="text-slate-400 text-xs">Recibe sonido y notificación nativa en el chat global.</p>
          </div>
        </div>
        <div className="shrink-0 w-full sm:w-auto mt-2 sm:mt-0">
          {permisoNotificaciones === 'granted' ? (
            ...
            <button onClick={solicitarNotificacionesManual} className="w-full sm:w-auto px-5 py-2.5 bg-blue-500/10 border border-blue-500/30 text-blue-400 hover:bg-blue-500/20 rounded-xl font-semibold transition-all text-sm">Activar Alertas</button>
          )}
        </div>
      </div>
      ...
      <div className="bg-[#141824] p-5 rounded-2xl border border-white/10 shadow-md flex flex-col sm:flex-row items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4 w-full">
          ...
          <div className="text-left">
            ...
            <p className="text-slate-400 text-xs">Usa tu huella para iniciar sesión rápidamente y seguro.</p>
          </div>
        </div>
        <div className="shrink-0 w-full sm:w-auto mt-2 sm:mt-0">
          {!tieneHuella ? (
            <button onClick={registrarHuellaGlobal} className="w-full sm:w-auto px-5 py-2.5 bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20 rounded-xl font-semibold transition-all text-sm">Activar Huella</button>
          ) : (
            <button onClick={quitarHuellaGlobal} className="w-full sm:w-auto px-5 py-2.5 bg-red-500/10 border border-red-500/30 text-red-400 hover:bg-red-500/20 rounded-xl font-semibold transition-all text-sm">Desactivar</button>
          )}
        </div>
      </div>
    </div>
